      ...
      <header className="border-b border-slate-200 px-4 pb-4 pt-4 dark:border-white/10 sm:px-5">
        ...
        {!showArchived && archivedCount > 0 && (
          <button
            type="button"
            onClick={onToggleArchived}
            className="mt-3 flex w-full items-center gap-3 rounded-2xl px-3 py-2.5 text-left text-sm font-bold text-slate-600 transition hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/5"
          >
            <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-100 text-slate-500 dark:bg-white/5 dark:text-slate-300">
              <Archive size={16} />
            </span>

            <span className="flex-1">Archivados</span>

            <span className="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-black text-slate-600 dark:bg-white/10 dark:text-slate-300">
              {archivedCount}
            </span>
          </button>
        )}
