...
type ShareMessageModalProps = {
  ...
  onShare: (friendIds: string[], conversationIds: string[]) => Promise<void>;
};
...
export function ShareMessageModal({
  ...
  const totalSelected =
    selectedFriendIds.length + selectedConversationIds.length;
  ...
  async function submit(): Promise<void> {
    if (busy || totalSelected === 0) {
      return;
    }

    await onShare(selectedFriendIds, selectedConversationIds);
  }
                ...
                {visibleFriends.map((friend) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => toggleFriend(friend.usuario_id)}
                      disabled={busy}
            ...
            {groups.length === 0 ? (
              ...
              <div className="space-y-1">
                {groups.map((conversation) => {
                  const selected = selectedConversationIds.includes(
                    conversation.id,
                  );
                  ...
                  return (
                    <button
                      key={conversation.id}
                      type="button"
                      onClick={() => toggleGroup(conversation.id)}
                      disabled={busy}
                      className="flex w-full items-center gap-3 rounded-2xl px-3 py-2.5 text-left transition hover:bg-slate-100 disabled:opacity-60 dark:hover:bg-white/5"
                    >
                      <Avatar
