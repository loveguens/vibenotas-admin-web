...
import type { ChatTab, Conversation } from "../features/types";
...
{loading ? (
  ...
  uniqueOrderedConversations.map((conversation, index) => (
    <ConversationItem
      ...
      selected={Number(conversation.id) === Number(selectedConversationId)}
      onClick={() => onSelect(Number(conversation.id))}
    />
  ))
)}
